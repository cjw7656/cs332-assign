  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = {
            if(initial.isEmpty) Stream()
              ...
              val new_explored=explored+initial.head._1;
              val new_neigh=newNeighborsOnly(neighborsWithHistory(initial.head._1,initial.head._2),new_explored)
              new_neigh++from(initial.tail ++ new_neigh, new_explored)
            }
           }
